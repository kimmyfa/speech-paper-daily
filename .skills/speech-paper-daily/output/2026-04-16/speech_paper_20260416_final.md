...
**共收录**：5篇 | **语音大模型**：1篇 | **语音前端**：1篇 | **语音安全**：3篇

今日 arXiv 语音相关论文共命中 5篇，按评分排序如下。

---

## 🔒 语音安全

## [1] ProSDD Learning Prosodic Representations for Speech Deepfake Detection

**arXiv ID**：2604.13229 | **方向**：语音安全
...
**发布日期**：2026-04-14 | **提交**：Interspeech 2026 | **论文**：https://arxiv.org/abs/2604.13229 | **PDF**：https://arxiv.org/pdf/2604.13229.pdf | **代码**：https://prosdd.github.io/ProSDD_website/ | **Demo**：暂无
...
**模型架构**：基于 XLS-R SSL backbone 构建，采用两阶段训练框架。Stage I 仅使用真实语音，通过监督掩码预测学习韵律表示。Stage II 联合优化韵律预测和欺骗分类。

**核心创新**：

- 说话人条件韵律目标：组合 speaker embedding（ECAPA-TDNN，192维）和 frame-level prosodic embedding（256维，集成 pitch、voice activity、energy）
- 监督掩码预测目标：使用 InfoNCE 损失区分正确和错误的 speaker-prosody pairs，采用 intra-speaker 和 inter-speaker negatives
- 两阶段策略：Stage I 先学习自然韵律变异性，Stage II 保持韵律监督作为辅助任务

**训练策略**：

- Stage I span masking（长度8，掩码概率0.25，温度τ=0.07），仅真实语音
- Stage II 掩码概率降至0.15，温度τ=0.1，联合损失α=1、β=0.2（前4 epoch）→ 0.05
...
- ASVspoof 2019 trained：0.42%（超越 XLSR-SLS 0.56%）
- ASVspoof 2024：EER 从 25.43% 降至 16.14%
- EmoFake：从 8.84% 降至 3.70%
- EmoSpoof-TTS：从 18.92% 降至 9.54%
- ASVspoof 2024 trained：EER 从 39.62% 降至 7.38%（相对减少81%）
...
理由：创新性强，从人类感知角度出发建模韵律变异性。实验非常充分，在多个 benchmark 上取得突破性进展（ASVspoof 2024 EER 从 39.62% 降至 7.38%，相对减少81%）。EmoFake 和 EmoSpoof-TTS 上相对减少约50%。已提交 Interspeech 2026，顶会水准，有实质贡献。

---

## 🤖 语音大模型

## [2] TimePro-RL Towards Fine-grained Temporal Perception

**arXiv ID**：2604.13715 | **方向**：语音大模型

**作者**：Yanfeng Shi, Pengfei Cai, Jun Liu, Qing Gu, Nan Jiang, Lirong Dai, Ian McLoughlin, Yan Song

**机构**：USTC + Singapore Institute of Technology

**发布日期**：2026-04-15 | **提交**：Interspeech 2026 | **论文**：https://arxiv.org/abs/2604.13715 | **PDF**：https://arxiv.org/pdf/2604.13715.pdf | **代码**：暂无 | **Demo**：暂无

### 📌 简介

当前大型音频-语言模型（LALM）在语义理解上表现优异，但在细粒度时间感知上存在瓶颈。本文提出 TimePro-RL 框架，通过音频侧时间提示和强化学习后训练显著提升时间对齐性能，在音频定位、声音事件检测和密集音频描述任务上取得突破。

### 🔧 技术方案

**模型架构**：基于 Qwen2-Audio 和 Qwen2.5-Omni 构建。核心设计 Audio-Side Time Prompt（ASTP），将时间戳编码为嵌入向量并交织到音频特征序列中作为时间坐标。时间戳嵌入采用语义初始化策略，使用对应数字字符串的子词嵌入平均值初始化，训练过程中冻结参数防止语义漂移。

**核心创新**：

- Audio-Side Time Prompt：在输入级别提供时间提示，减少推理难度并缓解幻觉
- 强化学习后训练：采用 Group Relative Policy Optimization（GRPO）优化时间对齐
- 自适应时间奖励机制：动态融合主奖励（Event-based F1）和辅助奖励（mIoU/METEOR），提升数据效率

**训练策略**：

- SFT 阶段：LoRA 微调（r=8，α=32），训练3 epoch，学习率 1×10^-5
- RL 阶段：仅1 epoch，使用10,200样本子集，组大小4，学习率 1×10^-6
- 时间戳 token 扩展：750个，覆盖0-30秒，步长0.04秒

### 📊 实验结果

数据集：Audio Grounding（61,862训练样本）、Sound Event Detection（15,041训练样本）、Dense Audio Captioning（92,443训练样本）
...
- 随机初始化时间戳嵌入 → SED Eb-F1 下降 2.9
- 语义初始化策略 → AG R@0.9 提升 1.7
- RL 训练 → AG R@0.9 从 35.8 提升至 39.8
...
## [3] SpeakerRPL v2 Robust Open-set Speaker Identification

**arXiv ID**：2604.13605 | **方向**：语音安全

**作者**：Zhiyong Chen, Shuhang Wu, Yingjie Duan, Xinkang Xu, Xinhui Hu

**发布日期**：2026-04-15 | **已被 ICASSP 2026 接收** | **论文**：https://arxiv.org/abs/2604.13605 | **PDF**：https://arxiv.org/pdf/2604.13605.pdf | **代码**：https://github.com/zhiyongchenGREAT/Few-shot-Robust-Speaker-TTS/tree/v2.1 | **Demo**：暂无

### 📌 简介

开放集说话人识别需要在识别已注册目标说话人的同时可靠检测未知说话人。本文改进 SpeakerRPL V1 框架，集成 LogitNorm 和互惠点学习，并提出模型融合策略减少少样本调优的随机性，在 Vox1-O 上 EER 从 1.28% 降至 0.09%（相对减少93%）。

### 🔧 技术方案

**模型架构**：基于预训练说话人基座模型构建。Reciprocal Points Loss（LRPL）结合判别性和 margin 约束。LogitNorm 归一化 logits 稳定开放集分类。自适应锚点扩展类别集合 K = Ktarget ∪ Ksynthetic ∪ Kadaptive，仅扩展 RP 而非 CP。

**核心创新**：

- 改进的开放集学习目标：联合优化 LRPL 和 LLogitNorm
- 自适应锚点学习：动态学习 reciprocal points，无需显式注册数据
- 模型融合策略：score-level averaging 多个 adapter 模型
- 模型选择策略：基于 CP 和 RP 的相似矩阵特征值方差

**训练策略**：

- 少样本调优：轻量级 MLP adapter，几分钟完成 GPU 训练
- 目标说话人增强：使用 GPT-SoVITSv2 生成合成数据
- 融合训练：30个 adapter 候选，丢弃底部33%模型
...
- Close-set：EER 从 1.28% 降至 0.09%（相对减少93%）
...
## [4] GatherMOS Few-Shot and Pseudo-Label Guided Speech Quality Evaluation

**arXiv ID**：2604.13528 | **方向**：语音前端

**作者**：Ryandhimas E. Zezario, Dyah A. M. G. Wisnu, Szu-Wei Fu, Sabato Marco Siniscalchi, Hsin-Min Wang, Yu Tsao

**发布日期**：2026-04-15 | **已被 IEEE ICASSP 2026 接收** | **论文**：https://arxiv.org/abs/2604.13528 | **PDF**：https://arxiv.org/pdf/2604.13528.pdf | **代码**：暂无 | **Demo**：暂无
...
**模型架构**：GatherMOS 使用 GPT-5 作为核心推理引擎，将声学特征和伪标签序列化为结构化文本提示。

**输入包括**：

- 轻量级声学描述器：RMS（能量）、ZCR（噪声程度）、clipping ratio、duration、13维 MFCC、mel-spectrogram 统计
- 伪标签：DNSMOS（1-5分）和 VQScore（0-1分）
...
- GatherMOS-ZS 版本增加 MFCC 和 spectrogram 特征
- 测试时使用 minibatch（10样本）评估，批次间重置 LLM session

### 📊 实验结果

数据集：VoiceBank-DEMAND（200 utterances，包括干净语音、4种噪声类型下的 0 dB 嘈杂语音、5种增强系统输出）
...
## [5] Classical Machine Learning Baselines for Deepfake Audio Detection

**arXiv ID**：2604.13400 | **方向**：语音安全

**作者**：Faheem Ahmad, Ajan Ahmed, Masudul Imtiaz

**发布日期**：2026-04-15 | **已被 The 35th IEEE Microelectronics Design and Test Symposium 接收（Oral）** | **论文**：https://arxiv.org/abs/2604.13400 | **PDF**：https://arxiv.org/pdf/2604.13400.pdf | **代码**：暂无 | **Demo**：暂无
...
**模型架构**：使用多种经典分类器 Logistic Regression、LDA、QDA、Gaussian Naive Bayes、SVM（RBF kernel）、GMM
...
涉及方向：语音大模型、语音前端、语音安全

推荐论文：ProSDD（9分）、TimePro-RL（8分）、SpeakerRPL v2（8分）
